...
function RadarChart(id, data, options) {
    ...
    //     }//for i
    // }//if
    cfg = ({ ...cfg, ...options})//{...options, ...cfg};
    //console.log(cfg)
    //If the supplied maxValue is smaller than the actual one, replace by the max in the data
    // var maxValue = Math.max(cfg.maxValue, d3.max(data, d => d3.max(d.map(o => o.value))));
    const maxValue = Math.max(cfg.maxValue, d3.max(data[0], d => d.value));

    const allAxis = data[0].map(d=>d.axis),//(data[0].map((i, j) => i.axis)),	//Names of each axis
    ...
    const rScale = d3.scaleLinear()//.linear()
        .range([0, radius])
        .domain([0, maxValue]);
    ...
    const radarLine = d3.lineRadial()//d3.svg.line.radial()
